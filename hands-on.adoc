...
Now let's take a loot at the configuration for the Debezium connector for Mysql, called _debezium_ in our stack. This connector will read the MySql binlog file and create, for each CRUD operation, an event that will be streamed to Kafka. The configuration is a Json file, containing different fields that will define how Debezium will create its messages.
[source,json]
----
include::kakfaConnectConfigs/debeziumConfig.json[]
----
...
[source,json]
----
include::kakfaConnectConfigs/neo4jConnectConfig.json[]
----
